...
└── lib/
    └── appwrite/          # Clients (client.ts, server.ts)
```
...
|-----|----------------|-------------|
| **Appwrite** | REST API via curl o SDK | Usa skill APPWRITE para BD. No hay MCP oficial. |
...
| `appwrite` | Collections, documentos, permisos, queries |
...
### 2. Mapear Estado de BD (via Appwrite SDK/REST)

Consulta Appwrite API para ver:
- Que tablas existen
...
## Template
SaaS Factory v4.0 (Next.js 16 + Appwrite)
...
## MCPs Activos
SI Appwrite | SI Next.js DevTools | SI Playwright
...
- **Appwrite SDK** = Tu conexion a la BD (no uses Supabase)
