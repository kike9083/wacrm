...
name: add-payments
description: |
  Integra sistema de pagos con Polar (Merchant of Record) en tu proyecto Next.js + Appwrite.
  ...
  Pre-requisito: /add-login (necesita auth + profiles en Appwrite).
...
1. `/add-login` ejecutado — busca `src/lib/appwrite/server.ts`. Si no existe, dile al usuario que ejecute `/add-login` primero.
