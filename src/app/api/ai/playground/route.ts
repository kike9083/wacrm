 ...
 *
 * Test-chat with the agent WITHOUT touching WhatsApp. Runs the exact
 * same path the auto-reply bot uses — knowledge-base retrieval +
...
export async function POST(request: Request) {
  try {
    ...
    console.error('[ai/playground] threw:', err)
    return NextResponse.json({ error: 'Failed to test the agent' }, { status: 500 })
  }
}
