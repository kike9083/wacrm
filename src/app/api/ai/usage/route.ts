 ...
 *
 * Token-spend summary for the BYO key over the last `days` (1–90,
 * default 30): totals, per-mode + per-model breakdowns, and a
 * zero-filled daily series for charting.
 */
export async function GET(request: Request) {
  try {
    ...
    let result
    try {
      result = await databases.listDocuments(DATABASE_ID, COLLECTIONS.aiUsageLog, [
        Query.equal('user_id', userId),
        Query.greaterThanEqual('created_at', since.toISOString()),
        Query.orderDesc('created_at'),
        Query.limit(MAX_ROWS + 1),
      ])
    } catch (err) {
      console.error('[ai/usage GET] fetch error:', err)
      return NextResponse.json({ error: 'Failed to load usage' }, { status: 500 })
    }

    const all = result.documents as unknown as UsageRow[]
    ...
    for (const r of rows) {
      ...
      byMode[mode].calls += 1
      byMode[mode].tokens += r.total_tokens ?? 0
    ...
    console.error('[ai/usage GET] threw:', err)
    return NextResponse.json({ error: 'Failed to load usage' }, { status: 500 })
  }
}
