...
export default function AutomationsPage() {
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? t('automations.failedToUpdate'))
      return
    }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t('automations.failedToDuplicate'))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t('automations.failedToDelete'))
      return
    }
    ...
    <div className="space-y-6">
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-800 bg-slate-900/40">
          ...
          <p className="mt-3 text-sm font-medium text-white">{t('automations.noAutomationsYet')}</p>
          <p className="mt-1 text-xs text-slate-400">
            {t('automations.pickTemplate')}
          </p>
        </div>
      ) : (
      ...
      <Dialog open={!!pendingDelete} onOpenChange={(v) => !v && setPendingDelete(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('automations.deleteDialogTitle')}</DialogTitle>
            <DialogDescription>
              {t('automations.deleteDialogDescription1')}{' '}
              <span className="text-white">{pendingDelete?.name}</span> {t('automations.deleteDialogDescription2')}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={onDuplicate}>
                ...
                {t('automations.duplicateAutomation')}
              </DropdownMenuItem>
