...
export default function BroadcastsPage() {
  ...
  async function fetchBroadcasts() {
    try {
      ...
      setError(err instanceof Error ? err.message : t('broadcasts.failedToLoad'));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {anySending && (
        <div
          role="progressbar"
          aria-label={t('broadcasts.broadcastInProgress')}
      ...
      {broadcasts.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-slate-800 bg-slate-900">
          ...
          <p className="text-sm font-medium text-white">{t('broadcasts.noBroadcastsYet')}</p>
          <p className="mt-1 text-xs text-slate-400">
            {t('broadcasts.noBroadcastsYetDescription')}
          </p>
          <Button
            onClick={() => router.push('/broadcasts/new')}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-slate-800 hover:bg-transparent">
                ...
                <TableHead className="hidden text-slate-400 lg:table-cell">{t('broadcasts.deliveryCol')}</TableHead>
                <TableHead className="hidden text-slate-400 lg:table-cell">{t('broadcasts.read')}</TableHead>
                <TableHead className="text-slate-400">{t('common.status')}</TableHead>
                <TableHead className="hidden text-slate-400 sm:table-cell">{t('broadcasts.dateCol')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
