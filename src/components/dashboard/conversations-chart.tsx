  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-800 bg-slate-900">
      <header className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-white">{t('dashboard.conversationsChart.title')}</h2>
          <p className="mt-0.5 text-xs text-slate-500">{t('dashboard.conversationsChart.subtitle')}</p>
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-slate-800/60 p-1">
          {[7, 30, 90].map((r) => (
            ...
            >
              {r} {t('dashboard.conversationsChart.days')}
            </button>
          ))}
        </div>
      </header>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={MessageSquare}
            title={t('dashboard.conversationsChart.noActivity')}
            hint={t('dashboard.conversationsChart.noActivityHint')}
          />
        ) : (
        ...
        >
          ...
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-blue-300">
              ...
              {hovered.incoming} {t('dashboard.conversationsChart.incoming')}
            </span>
            <span className="flex items-center gap-1.5 text-primary">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-primary" />
              {hovered.outgoing} {t('dashboard.conversationsChart.outgoing')}
            </span>
          </div>
        </div>
      )}
