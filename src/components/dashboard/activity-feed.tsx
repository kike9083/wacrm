  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900">
      <header className="flex items-center justify-between border-b border-slate-800 px-5 py-4">
        <h2 className="text-sm font-semibold text-white">{t('dashboard.recentActivity.title')}</h2>
        <Link
          href="/inbox"
          className="text-xs font-medium text-primary hover:text-primary/80"
        >
          {t('dashboard.recentActivity.viewAllLink')}
        </Link>
      </header>
      ...
      {loading || !items ? (
        ...
        <div className="p-5">
          <EmptyState
            ...
            hint={t('dashboard.recentActivity.noActivityHint')}
          />
        </div>
      ) : (
        <>
          ...
          <footer className="flex items-center justify-between border-t border-slate-800 px-5 py-3 text-xs">
            <span className="text-slate-500 tabular-nums">
              {t('dashboard.recentActivity.showing')} {visible.length} {t('dashboard.recentActivity.of')} {totalLoaded}
              {totalLoaded === 50 ? '+' : ''}
            </span>
            <div className="flex items-center gap-1">
              <span className="mr-1 text-slate-500">{t('dashboard.recentActivity.show')}</span>
