...
describe('generateReply — OpenRouter', () => {
  it('calls the openrouter endpoint and returns the reply', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({
        choices: [{ message: { content: 'Routing works' } }],
    ...
    const res = await generateReply({
      config: config({ provider: 'openrouter', apiKey: 'sk-or-v1-x' }),
      systemPrompt: 'sys',
      messages: [{ role: 'user', content: 'Hi' }],
    })
  ...
  it('maps a 429 to a rate_limited AiError', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        errResponse(429, { error: { message: 'Rate limit' } }),
      ),
    )
    await expect(
      generateReply({
    ...
    ).rejects.toMatchObject({ code: 'rate_limited', status: 502 })
  })
})
