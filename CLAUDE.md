...
Usuario dice algo
    ...
    |       → Ejecutar skill ADD-LOGIN (Appwrite auth completo)
    |
    ...
    |
    ├── "Necesito algo de la base de datos" / "coleccion" / "query" / "metricas"
    |       → Ejecutar skill APPWRITE (estructura + datos + metricas)
    |
...
| 2 | `add-login` | Auth completa: Email/Password + Google OAuth + profiles + Appwrite Permissions |
...
| 7 | `appwrite` | Todo BD: crear collections, Permissions, queries, metricas, CRUD |
...
| Backend | Appwrite (Auth + Databases + Storage + Permissions) |
...
### Appwrite (Tu Backend)
```
Sin MCP oficial. Usar skill APPWRITE para REST API via curl o SDK de node-appwrite.
```
...
- SIEMPRE configurar Permissions en Collections de Appwrite
...
│   ├── ai/                   # AI Templates hub
│   ├── appwrite/             # BD completa: collections, documentos, metricas
